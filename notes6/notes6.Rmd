...
<br>
<br>
[S5] $\eqspace \ar(B)\AR(B^{12}) \big((1-B)^d(1-B^{12})^D X_n-\mu) = \ma(B)\MA(B^{12}) \epsilon_n$,
<br>
<br>
where $\{\epsilon_n\}$ is a white noise process, the intercept $\mu$ is the mean of the differenced process $\{(1-B)^d(1-B^{12})^D X_n\}$, and we have ARMA polynomials $\ar(x)$, $\AR(x)$, $\ma(x)$, $\MA(x)$ as in model [S1].
...
* To talk formally about these results, we'd better write down a model and some hypotheses. Writing the data as $\data{x_{1:N}}$, collected at years $t_{1:N}$, the model we have fitted is
$$ (1-\ar_1 B)(X_n - \mu - \beta t_n) = \epsilon_n,$$
...
### Question: What other supplementary analysis analysis could you do to strengthen your conclusions?
