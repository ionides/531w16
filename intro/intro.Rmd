...
* A note on notation:
    ...
    + We use lower case Roman letters ($x$, $y$, $z$, $\dots$) to denote fixed numbers. Thus, these symbols refer to data or function of the data (i.e., statistics).
...
However, the residuals are close to uncorrelated, as we can check by plotting their pairwise sample correlations at a range of lags. This is called the sample autocorrelation function, or ACF, which is computed for the residual time series $\{r_t\}$ as
