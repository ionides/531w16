...
* Then, we can generate $N$ trajectories of length $T$, $x_{t,k}$, $k=1\,\dots,N$, $t=1,\dots,T$.
...
$$w_j=\prod_{n=1}^{N} f_{X_n|X_{n-1}}(x_{n,j}|x_{n-1,j},\theta),$$
then we have
$$\begin{eqnarray}
\lik(\theta) 
&\approx&
 \frac{1}{J}\,\sum_{j=1}^{J} \frac{\lik_j(\theta)}{w_j} 
\\
&=& 
\frac{1}{J}\,\sum_{j=1}^{J}\!\frac{\prod_{n=1}^{N} f_{Y_n|X_n}(y^*_n|x_{n,j}\params \theta)\,f_{X_n|X_{n-1}}(x_{n,j}|x_{n-1,j}\params \theta)}{\prod_{n=1}^{N} f_{X_n|X_{n-1}}(x_{n,j}|x_{n-1,j}\params \theta)} 
\\
&=& \frac{1}{J}\,\sum_{j=1}^{J}\!\prod_{n=1}^{N} f_{Y_n|X_n}(y^*_n|x_{n,j}\params \theta).
\end{eqnarray}$$
...
* Fortunately, we can compute the likelihood for a POMP model by a much more efficient algorithm than direct Monte Carlo integration. We proceed by factorizing the likelihood in a different way.
$$\lik(\theta)=f_{Y_{1:N}}{y^*_{1:N}\params \theta}
=\prod_{n}\,f_{Y_N|Y_{1:N-1}}{y^*_n|y^*_{1:n-1}\params\theta} 
=\prod_{n}\,\int_{x_{n}}\,f_{Y_n|X_n}{y^*_n|x_n\params\theta}\,f_{X_n|Y_{1:n-1}}{x_n|y^*_{1:n-1}\params\theta}\, dx_{n}.$$

* Recall the prediction and filtering formula, using the Markov property to obtain
...
f_{Y_n|X_n}(y^*_{n}|\tilde x_{n}\params\theta)\,f_{X_n|Y_{1:n-1}}(\tilde x_{n}|y^*_{1:n-1}\params\theta)\, d\tilde x_n}.
\end{eqnarray}$$
...
First, let's try deterministic optimization of a rough function.
