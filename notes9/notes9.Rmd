...
$$  f_{X_n|X_{n-1}}(x_n\given x_{n-1})= f(y_n\given x_n).$$
...
$$f_{X|Y,Z}(x\given y,z)= \frac{ f_{Y|X,Z}(y\given x,z)\, f_{X|Z}(x\given z)}{f_{Y|Z}(y\given z)}.$$
...
$$f_{X|Y,Z}(x\given y,z)= \frac{f_{XY|Z}(x,y\given z)}{f_{Y|Z}(y\given z)}.$$
...
= \prod_{n=1}^N f_{Y_{n}}(\data{y_{1:N}})
f_{Y_{n}|Y_{1:n-1}}(\data{y_n}\given \data{y_{1:n-1}})$.
<br><br>
...
f_{Y_{n+1:N}|X_{n}}(\data{y_{n+1:N}}\given x_n)
= \int f_{Y_{n+1:N}|X_{n+1}}(\data{y_{n+1:N}}\given x_{n+1}) \, 
