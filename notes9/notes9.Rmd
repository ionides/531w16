...
* Spline smoothing is a stanrdard method to smooth scatter plots and time plots.

* For example, `smooth.spline` in R.

* A **smoothing spline** for an equally spaced time series $\data{y_{1:N}}$ collected at times $t_{1:N}$ is the sequence $x_{1:N}$ minimizing the **penalized sum of squares (PSS)** defined as

<br>

[SS1] $\myeq{
  \mathrm{PSS}(x_{1:N}\params\lambda)}
  = \sum_{n-1}^N(\data{y_n}-x_n)^2 + \lambda\sum_{n=3}^T(\Delta^2 x_n)^2
}$,
...
* Note that $\{X_n,Y_n\}$ defined in [SS2] is not quite an LG-POMP model. However, we can use  $\{X_\}$ and $\{Y_n\}$ to build an LG-POMP model. How?
...
* Suppose that initial conditions are irrelevant (they could be either unknown parameters or an improper Gaussian distribution with infinite variance).  Then, noting that $\{\Delta^2 X_{1:N}\}$ and $\{Y_n-X_n\}$ are independent Normal random variables with mean zero and variances $\sigma^2/\lambda$ and $\sigma^2$, we have

<br>

[SS3] $\myeq{
\log f_{X_{1:N}Y_{1:N}}(x_{1:N},y_{1:N} \params \sigma,\lambda) = \frac{-1}{2\sigma^2} \sum_{n-1}^N(y_n-x_n)^2 +\frac{-\lambda}{2\sigma^2}  \lambda\sum_{n=3}^T(\Delta^2 x_n)^2 + C
}$.
...
$$\begin{eqnarray}
\arg\max_{x_{1:N}} \mathrm{PSS}(x_{1:N}\params\lambda),
&=& 
...
\\
&=& \E[X_{1:N}\given Y_{1:N}=\data{y_{1:N}} \params  \sigma,\lambda).
\end{eqnarray}$$
...
* If $X\sim N\big[\mu_X,\Sigma_X\big]$ has a probability density function then it is given by 
$$f_X(x) = \frac{1}{(2\pi)^{d_X/2}|\Sigma_X|}\exp\left\{-\frac{x\, \big[\Sigma_X\big]^{-1} \, x^\transpose \big}{2}\right\}.$$
This happens if and only if $\Sigma_X$ is invertible.
...
[KF2] $\eqspace \begin{eqnarray}
X_{n}\given Y_{1:n-1}=y_{1:n-1} &\sim& N\big[ \mu_{n}^P(y_{1:n-1}),\Sigma_n^P\big],
\\
X_n\given Y_{1:n}=y_{1:n} &\sim& N\big[ \mu_n^F(y_{1:n-1}),\Sigma_n^F\big],
\\
X_n\given Y_{1:N}=y_{1:N} &\sim& N\big[ \mu_n^S(y_{1:N}),\Sigma_n^F\big].
\end{eqnarray}$
...
$\mu_n^P(\data{y_{1:n-1}})=\E\big[X_n\given Y_{1:n-1}=\data{y_{1:n-1}}\big]$ is the **one-step prediction mean** for time $t_n$. It is an arbitrary decision we have made to call this the prediction for time $t_n$ (the time for which the prediction is being made) rather than for time $t_{n-1}$ (the time at which the prediction for time $t_n$ becomes available).

<br>

$\Sigma_n^P(\data{y_{1:n-1}})=\var\big(X_n\given Y_{1:n-1}=\data{y_{1:n-1}}\big)$ is the **one-step prediction variance** for time $t_n$. To make this terminology work for general POMP models as well as for LG-POMP models, we have included the dependence on $\data{y_{1:n-1}}$.

<br>

$\mu_n^F(\data{y_{1:n}})=\E\big[X_n\given Y_{1:n}=\data{y_{1:n}}\big]$ is the **filter mean** for time $t_n$.

<br>

$\Sigma_n^F(\data{y_{1:n}})=\var\big(X_n\given Y_{1:n}=\data{y_{1:n}}\big)$ is the **filter variance** for time $t_n$.

<br>

$\mu_n^S(\data{y_{1:N}})=\E\big[X_n\given Y_{1:N}=\data{y_{1:N}}\big]$ is the **smoothing mean** for time $t_n$. 

<br>

$\Sigma_n^S(\data{y_{1:N}})=\var\big(X_n\given Y_{1:N}=\data{y_{1:N}}\big)$ is the **smoothing variance** for time $t_n$.
...
[KF6] $\myeq{
  \mu_{n}^F(y_{1:n}) = \mu_{n}^P(y_{1:n-1}) + \Sigma_n^F \matB^\transpose_n\covmatY_n^{-1}\big\{y_n - \matB\mu_n^P(y_{1:n-1})\big\}
}$.
