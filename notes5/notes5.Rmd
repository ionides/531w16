...
* The whole parameter space on which the model is defined is $\Theta\subset\R^P$. 
...
\end{eqnarray}$$
defined via two nested parameter subspaces, $\Theta^{\langle 0\rangle}\subset \Theta^{\langle 1\rangle}$, with respective dimensions $P^{\langle 0\rangle}< P^{\langle 1\rangle}\le P$.
...
$$ 
\ell^{\langle 1\rangle} - \ell^{\langle 0\rangle} \approx (1/2) \chi^2_{P^{\langle 0\rangle}- P^{\langle 1\rangle},
$$
where $\chi^2_p$ is a chi-squared random variable on $p$ degrees of freedom and $\approx$ means "is approximately distributed as."
...
$$ \mbox{the estimated parameter $\data{\theta}$ does not lead us to reject a null hypothesis of $\theta=\theta^{\langle 0\rangle}$ at the 5\% level}$$
$$\Updownarrow$$
$$\mbox{$\theta^{\langle 0\rangle}$ is in a 95\% confidence interval for $\theta$}$$
...
$$ AIC = -2 \times \loglik(\data{\theta}) + 2P$$
"Minus twise the maximized log likelihood plus twice the number of parameters."
...
* Suppose we are in a situation in which we wish to choose between two nested hypotheses, with dimensions $P^{\langle 0\rangle}< P^{\langle 1\rangle}$. Suppose the Wilks approximation is valid.
...
* Evaluate this expression for $P^{\langle 1\rangle} - P^{\langle 0\rangle}=1$.
...
* Is this non-invertibility a problem? Let's investigate a little, using profile and bootstrap methods. The claimed standard error on the MA1 coefficient, from the Fisher information approach used by `arima` is small. First, we can see if the profile likelihood of this parameter is in agreement.
