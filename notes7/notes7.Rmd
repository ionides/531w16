...
\\
S_n &=& \frac{1}{\sqrt{N}}\sum_{k=1}^N X_k\sin(\omega_n k) \mbox{ for $1\le n\le N/2$}.
\end{eqnarray}$$
...
\\
\data{s_n} &=& \frac{1}{\sqrt{N}}\sum_{k=1}^N \data{x_k}\sin(\omega_n k) \mbox{ for $1\le n\le N/2$}.
\end{eqnarray}$$
...
$$ \data{z_n} = \left\{
\begin{array}{ll}
\data{y_n} \big(1-\cos(\pi n/Np)\big)/2 & \mbox{ if $n< Np$ }
\\
\data{y_n}  & \mbox{ if $Np \le n \le N(1-p)$ }
\\
\data{y_n} \big(1-\cos(\pi [N+1-n]/Np)\big)/2 & \mbox{ if $N(1-p)<n< N$ }
\end{array}\right.
$$
...
* If the time series consists of equally spaced observations with $\t_{n}-t_{n-1}=\Delta $years, we say that the frequency is $\omega/\Delta$ **cycles per year**. 
