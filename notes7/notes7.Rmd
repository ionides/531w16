...
* Got it! We have an explantion of the trend that makes scientific sense. However, the model is prefered by AIC but is not quite statistically significant tested against a null of white noise via the t-statistic for the estimated coefficient:

```{r glob_temp_fit}
summary(lm_global)
```

* Perhaps we now have a reasonable understanding of Ann Arbor January low temperatures: random, white noise, variation around the global mean temperature trend.
