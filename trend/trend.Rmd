...
* The **estimate** of the mean function is the value of the estimator when applied to the data. Here, we write this as
$$ \data{\mu_n^*} = \hat\mu_n(\data{x_{1:N}}).$$

* We call $\data{\mu_n}$ an **estimated mean function** or **estimated trend**.

*  For example, sometimes we suppose $\mu_n=\mu$, so the mean is constant. In this case, the model is called **mean stationary**. Then, we might estimate $\mu$ using the mean estimator,
$$\hat\mu(x_{1:N})=\frac{1}{N}\sum_{k=1}^N x_k.$$
In this case, the corresponding estimate $\data{\mu}$ is the sample mean.
...
* There is value in relatively simple statistical analysis, which can at least help to tell us what evidence there is for how things are changing. 
...
$$[L2]\quad\quad X = Z\beta + \zeta, \quad \quad \zeta \sim N[0,\Gamma].$$
...
$$\var\big[\hat \beta_{OLS}(X_{1:N})\big] = (Z^\tranpose Z)^{-1} Z^\tranpose \Gamma Z(Z^\tranpose Z)^{-1}.$$
